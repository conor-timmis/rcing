...
function renderZmiHighlights(options, summary) {
  ...
  if (summary.netGpPerEssence != null) gpMetaParts.push(`${formatGp(summary.netGpPerEssence)}/ess net`);

  const gpCard = gpValue != null
    ? tabHighlightCard({
        kind: "gp",
        label: gpLabel,
  ...
  const xpMeta = summary.xpPerEssence != null
  ...
  container.innerHTML = gpCard + tabHighlightCard({
    kind: "xp",
    label: "Net XP/hr",
    titleHtml: "Ourania altar",
...
function renderZmi(prices) {
  ...
  if (throughput) {
    throughput.textContent =
      `${summary.pouch.name} · ${summary.tripsPerHour.toFixed(1)} trips/hr · ${summary.essencesPerHour.toLocaleString()} ess/hr`;
  }
  if (setupSummary) {
    const badges = [];
    ...
    setupSummary.textContent =
      ...
      ` · ${options.tripSeconds}s · ${summary.essencesPerHour.toLocaleString()} ess/hr${joinBadges(badges)}`;
  }
}

function bindZmiControls(onUpdate) {
  ...
  for (const id of [
    "zmi-eye-toggle", "zmi-pouch-setup", "zmi-daeyalt-toggle",
    "zmi-diary-toggle", "zmi-supply-costs-toggle",
  ]) {
...
const loadZmi = initPriceTab({
  statusId: "zmi-status",
  bindControls: bindZmiControls,
  render: renderZmi,
});
